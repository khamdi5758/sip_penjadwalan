...
class DataJadwal extends CI_Controller
{
	...
	public function index()
	{
		$data = [
			...
			'ruang' => $this->Ruang_Model->getAllData(),
			'mapel' => $this->Mapel_Model->getAllData(),
			'hari' => $this->RangeHari_Model->getAllData(),
			...
			'rangeHari' => $this->RangeHari_Model->getAllData()
		];
	...
	public function indexcek()
	{
		...
		$this->load->view('templates/header');
		$this->load->view('templates/sidebar');
		$this->load->view('jadwal/cek', $data);
		$this->load->view('templates/footer');
	}
	...
	public function ubah($id)
	{
		...
		$this->form_validation->set_rules("jampel", "Jam ke ", "required");
		if ($this->form_validation->run() == FALSE) {
			$data['ubah'] = $this->Jadwal_Model->detail_data($id);
			$data['kelas'] = $this->Kelas_Model->getAllData();
			$data['ruang'] = $this->Ruang_Model->getAllData();
			$data['mapel'] = $this->Mapel_Model->getAllData();
			$data['hari'] = $this->RangeHari_Model->getAllData();
			$data['penjadwalan'] = $this->Jadwal_Model->getAllDataPenjadwalan();
			$data['jadwal'] = $this->Jadwal_Model->getAllData();
			$data['kelas'] = $this->Kelas_Model->getAllData();
			$data['rangeJam'] = $this->RangeJam_Model->getAllData();
			$data['rangeHari'] = $this->RangeHari_Model->getAllData();
			...
			$this->Jadwal_Model->ubah_data();
			$this->session->set_flashdata('flash_jadwal', 'DiUbah');
			redirect('DataJadwal');
		}
	}
	...
	{
		$this->Jadwal_Model->hapus_data($id_map);
